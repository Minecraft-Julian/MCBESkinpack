...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!form) {
    console.error('packForm not found');
    return;
  }
  if (!statusEl) {
  ...
  if (!addSkinBtn) {
    console.error('addSkinBtn not found');
    return;
  }
  if (!languageSelect) {
  ...
  function createSkinEntry({name = '', file = null} = {}) {
    ...
    nameLabel.htmlFor = `skin-name-${idx}`;
    const nameInput = document.createElement('input');
    nameInput.type = 'text';
    nameInput.id = `skin-name-${idx}`;
    ...
    fileLabel.htmlFor = `skin-file-${idx}`;
    const fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.id = `skin-file-${idx}`;
    ...
    typeLabel.htmlFor = `skin-type-${idx}`;
    const typeInput = document.createElement('input');
    typeInput.type = 'text';
    typeInput.id = `skin-type-${idx}`;
    ...
    removeBtn.addEventListener('click', () => {
      if (skinsContainer.contains(root)) {
  ...
  form.addEventListener('submit', async (ev) => {
    ev.preventDefault();
    if (statusEl) statusEl.textContent = 'Erzeuge Skinpack...';
    ...
    if (skins.length === 0) {
      if (statusEl) statusEl.textContent = 'Bitte mindestens einen vollständigen Skin (Name + PNG) hinzufügen.';
      return;
    }

    try {
      // Read skin files into array buffers
      if (statusEl) statusEl.textContent = 'Lese Skin-Dateien...';
      const skinBuffers = [];
      ...
      if (statusEl) statusEl.textContent = 'Erzeuge ZIP-Struktur...';
      const zip = new JSZip();
      ...
      if (statusEl) statusEl.textContent = 'Packe Dateien...';
      ...
      if (statusEl) statusEl.textContent = `Fertig: ${filename} — Download gestartet.`;
    } catch (err) {
      console.error(err);
