// Kombinierte Version mit Three.js-Vorschau + Fullscreen-Viewer
// Minimalste Änderung gegenüber deiner bisherigen Seite:
// - Beim Upload: prüft .png und Auflösung (64x32, 64x64, 128x128)
// - Wenn ein Skin hochgeladen wird, zeigt die Vorschau ein langsam rotierendes 3D-Modell (Three.js)
// - Klick auf Vorschau öffnet Fullscreen-Viewer (OrbitControls) zum Drehen; Klick außerhalb schließt
...
// Create placeholder PNG with big "X" (returns ArrayBuffer + dataUrl)
...
function makeMiniRenderer(canvas, imageUrl) {
  ...
  // simple model: head + torso (sufficient for preview)
...
// Fullscreen viewer (Three.js + OrbitControls)
...
// ----------------- Main app logic (kept mostly like before) -----------------
const LANGS = [
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // create DOM entry and mini renderer
  function createSkinEntryDOM(skin) {
    const entry = document.createElement('div');
    entry.className = 'skin-entry';
    entry.dataset.id = skin.id;

    // preview canvas (Three.js will render into it when possible)
    const previewWrap = document.createElement('div');
    previewWrap.className = 'preview';
    const canvas = document.createElement('canvas');
    // set reasonable internal resolution for renderer
    ...
    fileInput.addEventListener('change', async () => {
      ...
      // dispose existing mini renderer if any
    ...
    // initialize mini renderer from dataUrl or placeholder
    (async () => {
    ...
    // click preview -> fullscreen viewer
  ...
  // add new skin entry; open file dialog synchronously if requested
  async function addNewSkinEntry(openFileDialog = false) {
    ...
    if (openFileDialog) {
      try {
        fileInput && fileInput.click(); // must be in user gesture
        console.log('[mcbe] fileInput.click() called');
      } catch (e) { console.warn('[mcbe] fileInput click failed', e); }
    }
  }

  // pre-generate placeholders and fill UI
  ...
  // build pack (unchanged logic, uses uploaded buffers or placeholders)
  ...
  regenBtn.addEventListener('click', async () => {
    ...
    entries.forEach(entry => {
      ...
      const s = skinsData.find(x => x.id === id);
      if (!s) return;
      // re-create DOM by disposing renderer and re-initializing
      const canvas = entry.querySelector('canvas');
      const prev = miniRenderers.get(canvas);
      if (prev && prev.dispose) prev.dispose();
      // re-init mini renderer with new placeholder
  ...
  // viewer overlay close on outside click
